 ...
 */
class GitreleasesCommand extends AbstractCommand implements RequiresEnvironment
{
    ...
    public function run()
    {
        ...
        if (count($hosts) == 0) {
            ...
            foreach ($hosts as $host) {
                ...
                switch ($subcommand) {
                    case 'list':
                        $task = Factory::get('gitreleases/list', $this->getConfig());
                        $task->init();
                        $result = $task->run();
                        break;
                    ...
                    case 'rollback':
                        ...
                        $task = Factory::get('gitreleases/rollback', $this->getConfig());
                        $task->init();
